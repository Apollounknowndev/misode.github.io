...
import { FloatProvider, HolderSet, IntProvider } from './Common.js'

export function initConfiguredFeatures(schemas: SchemaRegistry, collections: CollectionRegistry, version: VersionId) {
  ...
  const LiquidSettings: NodeChildren = version === '1.21' ? {
  ...
  schemas.register('configured_feature', Mod(ObjectNode({
    type: StringNode({ validator: 'resource', params: { pool: 'worldgen/feature' as any } }),
    config: ObjectNode({
      ...
      [Case]: {
        'lithostitched:dungeon': {
          ...
          radius: Opt(IntProvider({ min: 1, max: 16 })),
          max_chests: Opt(NumberNode({ integer: true, min: 0 })),
          spawner_entity: Opt(ListNode(
            Mod(ObjectNode({
              data: StringNode({ validator: 'resource', params: { pool: 'entity_type' } }),
          ...
          floor_provider: Opt(Reference('block_state_provider')),
          wall_provider: Opt(Reference('block_state_provider')),
        ...
        'lithostitched:well': {
          ...
          slab_provider: Opt(Reference('block_state_provider')),
          fluid_provider: Opt(Reference('block_state_provider')),
          ground_provider: Opt(Reference('block_state_provider')),
          suspicious_provider: Opt(Reference('block_state_provider')),
          suspicious_block_placements: Opt(IntProvider({ min: 0, max: 4 })),
          suspicious_loot_table: Opt(StringNode()),
        },
