...
export function initMisc(schemas: SchemaRegistry, collections: CollectionRegistry) {
  ...
  let placementModifierTypes = [
		...collections.get('worldgen/placement_modifier_type')
	]

  placementModifierTypes.splice(11, 0, 'lithostitched:offset')

	collections.register(`worldgen/placement_modifier_type`, placementModifierTypes)
  ...
  schemas.register('decorator', ObjectNode({
    ...
    [Case]: {
      'lithostitched:offset': {
        x_offset: IntProvider({ min: -16, max: 16 }),
        y_offset: IntProvider(),
        z_offset: IntProvider({ min: -16, max: 16 }),
      },
